...
export default function MainSmartAccordion() {
  ...
  const smartSolutionsData = useMemo(() => {
    return accordionContent.items.map((item) => {
      const meta = smartSolutionsMeta.find((metaItem) => metaItem.id === item.id);
  ...
  useEffect(() => {
    ...
    const hasActiveTab = smartSolutionsData.some((item) => item.id === activeTab);

    if (!hasActiveTab) {
      setActiveTab(smartSolutionsData[0].id);
      setActiveIndex(0);
    }
  }, [smartSolutionsData, activeTab]);
